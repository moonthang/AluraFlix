...
const CardVideo = (props, video, categorias) => {
  const { abrirModal, fecharModal, setVideoSeleccionado, videos, setVideos } = useContext(FormularioContext);

  const { abrirVideoBanner } = useContext(FormularioContext)

  const abrirModalDeEdicao = () => {
    setVideoSeleccionado(props.video);
    abrirModal();
  ...
  return (
   ...
   >
      <div>
        <ImagenCapa corBorda={props.corBorda} corSombra={props.corSombra} src={props.video.imagem} alt="Imagen del video" />
      </div>
      <IconesEstilizados corSpan={props.corSpan}>
        <img onClick={handleClick} src={iconeEd} alt="Icono Editar" />
        <span onClick={handleClick} >EDITAR</span>
        <img onClick={ handleDelete} src={iconeEx} alt="Icono Eliminar" />
        <span onClick={ handleDelete}  >ELIMINAR</span>
      </IconesEstilizados> 
    </CardVideoEstilizado> 
  );
};
